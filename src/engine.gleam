...
pub type EngineMessage {
  ...
  SetUserOnline(user_id: UserId, is_online: Bool, reply: Subject(Result(Nil, String)))
  
  // Subreddit operations
  CreateSubreddit(name: SubredditName, creator: UserId, reply: Subject(Result(Nil, String)))
  JoinSubreddit(user_id: UserId, subreddit: SubredditName, reply: Subject(Result(Nil, String)))
  LeaveSubreddit(user_id: UserId, subreddit: SubredditName, reply: Subject(Result(Nil, String)))
  ...
  VotePost(post_id: PostId, user_id: UserId, is_upvote: Bool, reply: Subject(Result(Nil, String)))
  ...
  VoteComment(comment_id: CommentId, user_id: UserId, is_upvote: Bool, reply: Subject(Result(Nil, String)))
  GetPostComments(post_id: PostId, reply: Subject(Result(List(Comment), String)))
  
  // Direct message operations
  SendDirectMessage(from: UserId, to: UserId, content: String, reply: Subject(Result(MessageId, String)))
  GetDirectMessages(user_id: UserId, reply: Subject(Result(List(DirectMessage), String)))
  ReplyToDirectMessage(message_id: MessageId, from: UserId, content: String, reply: Subject(Result(MessageId, String)))
...
fn calculate_karma(state: EngineState, user_id: UserId) -> Int {
  ...
  let comment_karma =
    ...
    |> list.fold(0, fn(acc, comment) { acc + comment.upvotes - comment.downvotes })
...
fn handle_message(
  ...
  case message {
    RegisterUser(username, reply) -> {
      let user_id = "user_" <> username <> "_" <> string.inspect(dict.size(state.users))
      ...
      case dict.has_key(state.users, user_id) {
        ...
        False -> {
          ...
          let new_state = EngineState(..state, users: dict.insert(state.users, user_id, user))
    ...
    SetUserOnline(user_id, is_online, reply) -> {
      case dict.get(state.users, user_id) {
        Ok(user) -> {
          ...
          let new_state = EngineState(..state, users: dict.insert(state.users, user_id, updated_user))
          ...
          case dict.get(state.users, creator) {
            Ok(user) -> {
              let updated_user = User(
                ..user,
                subscribed_subreddits: [name, ..user.subscribed_subreddits],
              )
              let final_state = EngineState(
                ..new_state,
        ...
        Ok(subreddit) -> {
          case list.contains(subreddit.members, user_id) {
            ...
            False -> {
              ...
              let new_state = EngineState(
                ..state,
                subreddits: dict.insert(state.subreddits, subreddit_name, updated_subreddit),
              )
              ...
              case dict.get(state.users, user_id) {
                Ok(user) -> {
                  let updated_user = User(
                    ..user,
                    subscribed_subreddits: [subreddit_name, ..user.subscribed_subreddits],
                  )
                  let final_state = EngineState(
                    ..new_state,
      ...
      case dict.get(state.users, user_id) {
        Ok(user) -> {
          let feed =
            ...
            |> list.take(50)  // Limit to 50 posts
      ...
      case dict.get(state.posts, post_id) {
        Ok(post) -> {
          ...
          let updated_post = Post(
            ..post,
            comments: [comment_id, ..post.comments],
          )
              ...
              case dict.get(state.comments, parent_id) {
                Ok(parent) -> {
                  let updated_parent = Comment(
                    ..parent,
                    replies: [comment_id, ..parent.replies],
                  )
...
pub fn start() -> Result(actor.Started(Subject(EngineMessage)), actor.StartError) {
