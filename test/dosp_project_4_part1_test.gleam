...
pub fn user_registration_test() {
  ...
  process.send(engine_actor, engine.RegisterUser(username: "testuser", reply: reply))
...
pub fn subreddit_creation_test() {
  ...
  process.send(engine_actor, engine.RegisterUser(username: "creator", reply: register_reply))
  let assert Ok(Ok(user_id)) = process.receive(register_reply, 1000)
  
  // Create subreddit
  let sub_reply = process.new_subject()
  process.send(engine_actor, engine.CreateSubreddit(name: "test_sub", creator: user_id, reply: sub_reply))
...
pub fn post_creation_test() {
  ...
  // Register user
  let register_reply = process.new_subject()
  process.send(engine_actor, engine.RegisterUser(username: "poster", reply: register_reply))
  let assert Ok(Ok(user_id)) = process.receive(register_reply, 1000)
  ...
  process.send(engine_actor, engine.CreatePost(
    author: user_id,
    subreddit: "test_sub",
    title: "Test Post",
...
pub fn voting_test() {
  ...
  // Register user
  let register_reply = process.new_subject()
  process.send(engine_actor, engine.RegisterUser(username: "voter", reply: register_reply))
  let assert Ok(Ok(user_id)) = process.receive(register_reply, 1000)
  ...
  process.send(engine_actor, engine.VotePost(
    post_id: post_id,
    user_id: user_id,
    is_upvote: True,
    reply: vote_reply
...
pub fn engine_stats_test() {
  ...
  process.send(engine_actor, engine.RegisterUser(username: "user1", reply: register_reply1))
  let assert Ok(Ok(_)) = process.receive(register_reply1, 1000)
  
  let register_reply2 = process.new_subject()
  process.send(engine_actor, engine.RegisterUser(username: "user2", reply: register_reply2))
